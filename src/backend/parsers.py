...
def parse_udp_segment(data):
    ...
    return {"source_port": src_port, "destination_port": dest_port, "data": data[8:]}
...
def parse_ICMPv6(data):
    ...
    return {"ICMP_Type": icmp_type, "code": code, "data": data[2:]}
...
def parse_DNS(data):
    ...
    rcode = flags & 0x0F
    nbrquestions, answer_RRs, authority_RRs, additional_RRs = struct.unpack(
        "!HHHH", data[4:12]
    )
    ...
    for _ in range(answer_RRs):
        ...
        rrtype, rrclass, ttl, rdlength = struct.unpack(
...
def parse_DHCP(data):
    opcode, hardware_type, hardware_addr_len, nbrhops, transaction_id = struct.unpack(
        "!BBBB4s", data[:8]
    )
