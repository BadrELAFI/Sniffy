...
async def packet_sniffer(websocket):
    ...
    try:
        ...
        await websocket.send(
            json.dumps(
                {"error": "Permission denied. Run as administrator."}, default=json_default_handler
            )
        )
        return
    except Exception as e:
        print(f"Error creating socket: {e}")
        await websocket.send(json.dumps({"error": f"Socket error: {e}"}, default=json_default_handler))
        return
    ...
    while True:
        try:
            raw_data = await loop.sock_recv(conn, 65535)
            ...
            if protocol == 0x0800:  # IPv4
                ...
                if EXCLUDE_LOOPBACK and (
                    ipv4["source_ip"] in LOOPBACK_IPS
                    or ipv4["destination_ip"] in LOOPBACK_IPS
                ):
                    should_send = False
                    continue
                    ...
                    try:
                        ...
                        if lines and lines[0]:
                            if parsers.is_HTTP_Request(lines[0]):
                                ...
                                if (
                                    "method" in parsedHTTP
                                    and "path" in parsedHTTP
                                    and "version" in parsedHTTP
                                ):
                                    packet_out["HTTP"] = parsedHTTP
                            else:
                                parsedHTTP = parsers.parse_HTTP_Reply(lines)
                                if (
                                    "version" in parsedHTTP
                                    and "status" in parsedHTTP
                                    and "reason" in parsedHTTP
                                ):
                                    packet_out["HTTP"] = parsedHTTP
                    except Exception as e:
            ...
            await websocket.send(json.dumps(packet_out, default=json_default_handler))
